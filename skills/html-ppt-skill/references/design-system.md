...
/* ===== 正文字号阶梯（按页信息密度选用，防溢出） ===== */
/* 常规：封面/章节/单概念页 */
.body-text    { font-size: 1.05vw; line-height: 1.85; }
/* 双栏页、2×2 卡片页 —— 双栏布局优先用这个，别用 .body-text */
.body-compact { font-size: 0.92vw; line-height: 1.55; }
/* 列表项、状态说明 */
.small        { font-size: 0.85vw; line-height: 1.6; }
/* 脚注、提示条 */
.mini         { font-size: 0.78vw; line-height: 1.45; }
```

> ⚠️ 双栏布局页（左图右说明）正文一律用 `.body-compact`，不要用 `.body-text`，否则极容易撑爆单页。

### 流程图节点（代替图片，纯 CSS 组合）

流程/状态机/时序图用 `.fnode`（节点）+ `.arrow-i`（箭头）内联组合，配 `.acc/.gold/.green/.orange/.red/.gray` 语义色：

```css
.fnode {
  background: var(--c-surface); border: 1.5px solid var(--c-border);
  border-radius: 8px; padding: 0.7vw 1vw; text-align: center;
  font-size: 0.82vw; color: var(--c-text); line-height: 1.4;
}
.fnode b   { font-weight: 700; display: block; }
.fnode span{ font-size: 0.68vw; color: var(--c-muted); }

/* 语义配色 */
.fnode.acc   { border-color: var(--c-accent);  background: #EFF4FE; } /* 主流程 */
.fnode.gold  { border-color: #D97706; background: #FFF8E8; }         /* 主分支/关键 */
.fnode.green { border-color: #059669; background: #ECFDF5; }         /* 在线/成功 */
.fnode.orange{ border-color: #EA580C; background: #FFF5EA; }         /* 草稿/进行中 */
.fnode.red   { border-color: #DC2626; background: #FEF1F1; }         /* 异常/禁用 */
.fnode.gray  { border-color: #CBD5E1; background: #F1F5F9; color:#64748B; } /* 归档/占位 */

.arrow-row { display: flex; align-items: center; }
.arrow-i   { color: var(--c-accent); font-weight: 700; font-size: 1.3vw; padding: 0 0.3vw; }
```
...
#nav-dots {
  ...
  opacity:0.35; transition:opacity 0.3s;  /* 默认半透明，防遮挡 */
}
...
底部按钮 HTML 结构（`⏮` 开头 = btn-first，`⏭` 结尾 = btn-last，`⛶` 全屏 = btn-fs）：

```html
<div id="nav-arrows">
  <button class="arrow-btn" id="btn-first" title="回到开头">⏮</button>
  <button class="arrow-btn" id="btn-up" title="上一页">↑</button>
  <button class="arrow-btn" id="btn-dn" title="下一页">↓</button>
  <button class="arrow-btn" id="btn-last" title="跳到结尾">⏭</button>
  <button class="arrow-btn" id="btn-fs" title="全屏/退出全屏">⛶</button>
</div>
```
...
document.getElementById('btn-last').addEventListener('click', () => goTo(slides.length - 1));
...
> 说明：原模板里 `IntersectionObserver ... .observe = (()=>{...})()` 的写法有误，已替换为标准的「先创建 `io` 再逐个 `observe`」写法。
---

## 防溢出（重要）

`vw` 字号强依赖 16:9 视口，窗口更扁时内容会溢出被裁切。两层防御：

**① 字号阶梯**：双栏页用 `.body-compact`，见「基础排版」节。

**② 非 16:9 整体缩放（治本）**，覆盖所有内联字号：

```css
@media (min-aspect-ratio: 16/9) {
  .slide .pad {
    zoom: clamp(0.8, calc(100vh / (100vw * 0.5625)), 1.0);
  }
}
```

验证：用 Playwright 无头浏览器在多个视口（16:9 全屏 + 偏扁如 1600×700）逐页检测内容元素 `getBoundingClientRect().bottom` 是否超出 slide 高度；检测时要排除 `.glow` / `.grid-bg` / `.watermark` 等装饰元素（它们的出血是故意的）。
