...
### ⚠️ 防溢出（本 skill 最关键的一条）

`vw` 字号体系**强依赖 16:9 视口**。当浏览器窗口比 16:9 更扁（宽高比 > 16/9，即高度相对更小）时，`vw` 字体相对变大、页高 `56.25vw` 不变，会造成**内容溢出当页、被 `overflow:hidden` 裁切**。这是 HTML PPT 最常见的翻车点，必须从两个层面预防：

**① 内容层 —— 字号分级，按页信息密度选用**

```css
.body-text    { font-size:1.05vw; line-height:1.85; }  /* 常规：封面/章节/单概念页 */
.body-compact { font-size:0.92vw; line-height:1.55; }  /* 双栏页、2×2 卡片页 */
.small        { font-size:0.85vw; line-height:1.6;  }  /* 列表项、状态说明 */
.mini         { font-size:0.78vw; line-height:1.45; }  /* 脚注、提示条 */
```

双栏布局页（左图右说明）正文优先用 `.body-compact`，不要用 `.body-text`，否则很容易撑爆。

**② 视口防御层 —— 非 16:9 时整体缩放（治本）**

```css
/* 非标准 16:9 视口（更扁）自动整体缩放内容，覆盖所有内联字号 */
...
`zoom` 作用于内容容器 `.pad`，**等比缩放所有内容（含内联字号）**，比逐个改字号变量可靠。`clamp` 保证缩放不超 1.0、不低过 0.8。

> 检查方式：做完后用 Playwright 无头浏览器在**多个视口**（16:9 全屏 + 偏扁窗口如 1600×700）逐页检测内容元素 `getBoundingClientRect().bottom` 是否超出 slide 高度。纯靠肉眼看 `vw` 单位在窗口缩放时不可靠。
...
**导航防遮挡**：导航元素（圆点、箭头、首尾/全屏按钮）默认半透明，鼠标悬停时恢复清晰，避免一直挡住内容：

```css
#nav-dots, #nav-arrows { opacity: 0.35; transition: opacity 0.3s; }
#nav-dots:hover, #nav-arrows:hover { opacity: 1; }
```
...
2. **确认口径**（制作前问清，避免返工）：
   - 受众是谁、讲多深（零基础 / 技术 / 混合）
   - 呈现形式（是否允许出现代码 / 以流程图·状态机图·时序图为主）
   - **业务字段的真实语义**（若涉及领域概念，先回代码/文档确认真实含义，不要凭直觉类比；尤其"百分比/概率/比例"类字段最易讲错）
   - 规模（页数）
3. **选定主题**：从 `references/design-system.md` 选择或创建配色方案
4. **规划页面**：确定每页类型和内容大纲
5. **逐页编写**：每张 `.slide` 按内容量和页面类型选择合适布局，双栏页用 `.body-compact` 字号
6. **检查清单**（完成后必须自查）：
   - [ ] 每页都有 `class="slide"` 和 `id="slide-N"`
   - [ ] 每页底部有页码 `<span class="slide-num">N / Total</span>`
   - [ ] body 有 `scroll-snap-type: y mandatory`
   - [ ] 包含键盘导航 JS（含 Home/End）
   - [ ] 包含首尾跳转 ⏮⏭ 与全屏 ⛶ 按钮
   - [ ] 导航元素默认半透明、hover 恢复
   - [ ] 包含 `@media print` 样式
   - [ ] 所有字体大小用 `vw` 单位（非 `px`）
   - [ ] 包含非 16:9 视口的 `zoom` 防溢出规则
   - [ ] **多视口无溢出验证**（16:9 全屏 + 偏扁窗口，见上方防溢出节）
